    ...
    if(User.is_authenticated):
        if request.method=="POST":
            if request.POST.get("selected_user")=="Refresh Data":
                ...
                getProfile[0].last_updated=timezone.now() #datetime.now().astimezone(pytz.timezone('Asia/Kolkata')).replace(tzinfo=None)
                getProfile[0].save()
            ...
            if(Profile.objects.filter(user_id=request.user.pk).exists()):
                ...
                if(data1.status_code==200 & data2.status_code==200):
                    json_data1=data1.json()
                    json_data2=data2.json()
                    newprofile=Profile.objects.create(user=request.user,followers=json_data1['followers'],last_updated=timezone.now().replace(tzinfo=None))
                    for repos in json_data2:
